...
number1, number2 = getVideoDemensions(video_file)
bytes_list1 = []
bytes_list2 = []

# Convert the first integer to an 8-byte array
n = number1
while n > 0:
    byte = min(n, 255)
    bytes_list1.append(byte)
    n -= byte
while len(bytes_list1) < 8:
    bytes_list1.append(0)

# Convert the second integer to an 8-byte array
n = number2
while n > 0:
    byte = min(n, 255)
    bytes_list2.append(byte)
    n -= byte
while len(bytes_list2) < 8:
    bytes_list2.append(0)

# Write the byte arrays to a file
with open(minvFile, 'wb') as f:
    for byte1 in bytes_list1:
        byte1 = int(byte1).to_bytes(1, 'little')
        f.write(byte1)
    for byte2 in bytes_list2:
        byte2 = int(byte2).to_bytes(1, 'little')
        f.write(byte2)
